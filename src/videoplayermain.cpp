    ...
    void audio_callback(void *userdata ,Uint8 *stream,int len)
    {
        ...
        while (remaining > 0) 
        {
            ...
            if (audio_buf.data == nullptr || audio_buf.pos >= audio_buf.size)
            {
                //让回调函数处于等待状态
                audio_buf.waiting = 1;
    ...
    int main(int argc, char *argv[]){
        if(argc < 2){
            std::cout << "缺少输入内容，需要输入./myplayer和文件的地址名称" << std::endl;
            return -1;
        }
        ...
        int video_stream = -1;
        int audio_stream = -1;
        for (int i = 0; i < static_cast<int>(ctx->nb_streams); i++)
        {
        ...
        std::cout << "视频帧率：" << av_q2d(frame_rate) << " fps, 每秒延迟：" << frame_delay * 1000 << " 毫秒" << std::endl;
        ...
        if (!codec_cxt)
        {
            std::cout << "分配视频上下文失败" << std::endl;
            return -1;
        }
        avcodec_parameters_to_context(codec_cxt, codec_par);
        //把之前的流的信息都储存到编码器中
        if (avcodec_open2(codec_cxt, codec, nullptr) < 0)
        {
        ...
        if (audio_ctx && swr_ctx)
        {
            ...
            audio_buf.device_id = SDL_OpenAudioDevice(nullptr, 0, &want, nullptr, 0);
        ...
        while (av_read_frame(ctx, &pkt) >= 0)
        {
            ...
            if (audio_stream != -1 && pkt.stream_index == audio_stream && swr_ctx && audio_buf.device_id)
            {
                ...
                while (avcodec_receive_frame(audio_ctx, audio_frame) == 0)
                {
                    ...
                    uint8_t *out = nullptr;
                    //为输入的音频分配缓冲区的内存
                    av_samples_alloc(&out, nullptr, 2, out_samples, AV_SAMPLE_FMT_S16, 0);
                    ...
                    {
                        //计算缓存区的大小存放到size
                        int size = av_samples_get_buffer_size(nullptr, 2, samples, AV_SAMPLE_FMT_S16, 1);

                        //锁住互斥锁
                        SDL_LockMutex(audio_buf.mutex);
                        //有数据，上一层数据还没执行完
                        while(audio_buf.data != nullptr)
                        {
                            //等待，最多10ms
